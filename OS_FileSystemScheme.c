...
void OSFS_Init()
{
	DSTATUS DiskStatus = eDisk_Init(DRIVENUM);
	if (DiskStatus != 0) OS_KernelPanic(SDCARD_INITFAILED, "OSFS_Init");
	...
	{
		OS_KernelPanic(FILESYS_INITFAILED, "OSFS_Init Location 1");
	}
	...
	if (DataBitMap == 0 || InodeBitMap == 0) OS_KernelPanic(FILESYS_INITFAILED, "OSFS_Init location 4");
	...
	{
		OS_KernelPanic(FILESYS_INITFAILED, "OSFS_Init Location 2");
	}
	...
	{
		OS_KernelPanic(FILESYS_INITFAILED, "OSFS_Init Location 3");
	}
...
bool OSFS_Format()
{
	...
	uint32_t Status = StartCritical(); // Ensure nothing interrupts when messing around with the file system
	DSTATUS DiskStatus = eDisk_Init(DRIVENUM);
	if (DiskStatus != 0) OS_KernelPanic(SDCARD_INITFAILED, "OSFS_Init");
	...
	{
		OS_KernelPanic(FILESYS_FORMATFAILED, "Writing superblock to disk failed.");
	}
	...
	if (DataBitMap == 0 || InodeBitMap == 0) OS_KernelPanic(FILESYS_FORMATFAILED, "Could not create DataBitMap, InodeBitMap");
	...
	{
			OS_KernelPanic(FILESYS_FORMATFAILED, "OSFS_Format location 4");
	}
	...
	if (DataBitMap == 0 || InodeBitMap == 0) OS_KernelPanic(FILESYS_FORMATFAILED, "OSFS_Format location 2");
	...
	{
			OS_KernelPanic(FILESYS_FORMATFAILED, "OSFS_Format location 5");
	}
	...
	for (InodeIterator = 0; InodeIterator < TOTAL_INODE_SECTORS; InodeIterator++)
	{
		...
		{
			OS_KernelPanic(FILESYS_INITFAILED, "OSFS_Init Location 6");
		}
...
FILE* OSFS_Create(char* fileName)
{
	...
	FILE* fileToReturn = (FILE*) OS_NewAllocation(sizeof(FILE) * 1);

	if (fileToReturn == 0)
	{
		RecentError = FILE_INIT_FAILED;
		OS_DestroyAllocation(newFile, sizeof(INODE));
		return 0;
	}
...
FILE* OSFS_Open(char* fileName)
{
	INODE* createdFile = (INODE*) OS_NewAllocation(sizeof(INODE) * 1);
	...
	FILE* returnFile = (FILE*) OS_NewAllocation(sizeof(INODE) * 1);
...
bool OSFS_Close(FILE* fileToClose)
{
	...
	OS_DestroyAllocation(fileToClose->FileInode, sizeof(INODE));
	OS_DestroyAllocation(fileToClose, sizeof(FILE));
...
bool OSFS_Delete(char* fileName)
{
	...
	OS_DestroyAllocation(createdFile, sizeof(INODE));

	return TRUE;

}

uint32_t GetFileSize(FILE* fileToEval)
{
...
{
	DSTATUS DiskStatus = eDisk_Status(DRIVENUM);
	if (DiskStatus != 0) return FALSE;
	DRESULT WriteResult = eDisk_WriteBlock(InputBuffer, BlockNum);
...
bool _ReadFromFile(BYTE* OutputBuffer, uint64_t BlockNum)
{
	DSTATUS DiskStatus = eDisk_Status(DRIVENUM);
	if (DiskStatus != 0) return FALSE;
	DRESULT ReadResult = eDisk_ReadBlock(OutputBuffer, BlockNum);
...
uint32_t _GetNextFreeInode()
{
	...
	OS_KernelPanic(FILESYS_OUTOFINODES, "_GetNextFreeInodeandMark");
...
{
	if (InodeBitMap == 0) OS_KernelPanic(FILESYS_INITFAILED, "_MarkInodeAsOccupied");
...
{
	if (DataBitMap == 0) OS_KernelPanic(FILESYS_INITFAILED, "_MarkBlockAsOccupied");
	BitMap_SetBit(DataBitMap, BlockNum);
}

void _MarkBlockAsFree(uint32_t BlockNum)
{
	if (DataBitMap == 0) OS_KernelPanic(FILESYS_INITFAILED, "_MarkBlockAsFree");
...
void SerialListFiles()
{
	...
	while(TRUE)
	{
		...
		OS_SerialOutString(CurrentNode.FILE_NAME);
		OS_SerialOutString(" :: ");
		OS_SerialOutUDec(CurrentNode.BYTES_USED);
		OS_SerialOutString(" bytes.");
		OS_SerialOutString("\n");
...
void SerialPrintFile(FILE* fileToPrint)
{
	...
	BYTE* fileContents = OS_NewAllocation(filenode->BYTES_USED + 1);
	...
	if (fileContents)
	{
		...
		OS_SetOutputToSerial();
		printf((char*) fileContents);
		OS_DestroyAllocation(fileContents, filenode->BYTES_USED + 1);
	...
	else
	{
		...
		{
			OSFS_Read(fileToPrint, (BYTE*) &thisByte, 1, ByteIterator);
			OS_SerialOutString((char*) thisByte);
			memset(&thisByte, 0, 1);
		}
	}
}
